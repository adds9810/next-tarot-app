...
export async function POST(req: NextRequest) {
  ...
  const prompt = `다음 타로 카드 "${card}"를 기반으로, ${
    questionType === "today"
      ? "오늘 당신에게 속삭이는 별의 운세"
      : "당신의 질문에 대해 별들이 건네는 속삭임"
  }을 은유적이고 감성적인 문장으로 3~5줄 이내로 요약해줘. 카드의 상징과 현재 고민에 어울리는 현실적인 조언도 부드럽게 담아줘. 마지막 문장은 희망적인 여운으로 마무리해줘. 전체적인 어투는 별이 밤하늘에서 속삭이듯 부드럽고 시적인 느낌이면 좋아.`;
  try {
