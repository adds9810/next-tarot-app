...
export default function NewRecordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("session:", session); // 여기 확인 필요
      ...
      const { error } = await supabase.from("records").insert({
        ...
        main_cards: mainCards.map((card) => card.id),
        sub_cards: subCards.map((card) => card.id),
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <CardSelector
              ...
              onChange={(cards) => {
                handleMainCardsChange(cards);
                if (errors.mainCards) {
                  setErrors({ ...errors, mainCards: undefined });
                }
              }}
              maxCards={10}
              isMain={true}
